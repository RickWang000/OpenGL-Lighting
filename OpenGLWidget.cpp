...
void CoreFunctionWidget::loadConfig() {
    ...
    for (int i = 0; i < models.size(); ++i) {
        ...
        QVector3D ka(model["ka"].toArray()[0].toDouble(),
                     model["ka"].toArray()[1].toDouble(),
                     model["ka"].toArray()[2].toDouble());
        QVector3D kd(model["kd"].toArray()[0].toDouble(),
                     model["kd"].toArray()[1].toDouble(),
                     model["kd"].toArray()[2].toDouble());
        QVector3D ks(model["ks"].toArray()[0].toDouble(),
                     model["ks"].toArray()[1].toDouble(),
                     model["ks"].toArray()[2].toDouble());
        QVector3D F0(model["F0"].toArray()[0].toDouble(),
                     model["F0"].toArray()[1].toDouble(),
                     model["F0"].toArray()[2].toDouble());
        float ior = model["ior"].toDouble();
        int type = model["type"].toInt();
        ...
        modelsKs.append(ks);
        modelsF0.append(F0);
        modelsIor.append(ior);
        modelsType.append(type);
    }
}
...
void CoreFunctionWidget::setupShaders() {
    ...
    // 初始化光线追踪着色器
    success = rayTracingShaderProgram.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/shaders/raytracing.frag");
    if (!success) {
        qDebug() << "rayTracingShaderProgram addShaderFromSourceFile failed!" << rayTracingShaderProgram.log();
        return;
    }

    success = rayTracingShaderProgram.link();
    if (!success) {
        qDebug() << "rayTracingShaderProgram link failed!" << rayTracingShaderProgram.log();
    }
...
void CoreFunctionWidget::paintGL() {
    ...
    {
        glUniform3fv(rayTracingShaderProgram.uniformLocation("lightPos"), 1, &lightPos[0]);
        glUniform3fv(rayTracingShaderProgram.uniformLocation("viewPos"), 1, &this->cam.eye[0]);

        for (int i = 0; i < 3; ++i) {
            QString uniformName = QString("spheres[%1].center").arg(i);
            glUniform3fv(rayTracingShaderProgram.uniformLocation(uniformName.toStdString().c_str()), 1, &modelsPosition[i][0]);

            uniformName = QString("spheres[%1].radius").arg(i);
            glUniform1f(rayTracingShaderProgram.uniformLocation(uniformName.toStdString().c_str()), modelsScale[i].x());

            uniformName = QString("spheres[%1].material.ka").arg(i);
            glUniform3fv(rayTracingShaderProgram.uniformLocation(uniformName.toStdString().c_str()), 1, &modelsKa[i][0]);

            uniformName = QString("spheres[%1].material.kd").arg(i);
            glUniform3fv(rayTracingShaderProgram.uniformLocation(uniformName.toStdString().c_str()), 1, &modelsKd[i][0]);

            uniformName = QString("spheres[%1].material.ks").arg(i);
            glUniform3fv(rayTracingShaderProgram.uniformLocation(uniformName.toStdString().c_str()), 1, &modelsKs[i][0]);

            uniformName = QString("spheres[%1].material.shininess").arg(i);
            glUniform1f(rayTracingShaderProgram.uniformLocation(uniformName.toStdString().c_str()), modelsShininess[i]);

            uniformName = QString("spheres[%1].material.F0").arg(i);
            glUniform3fv(rayTracingShaderProgram.uniformLocation(uniformName.toStdString().c_str()), 1, &modelsF0[i][0]);

            uniformName = QString("spheres[%1].material.ior").arg(i);
            glUniform1f(rayTracingShaderProgram.uniformLocation(uniformName.toStdString().c_str()), modelsIor[i]);

            uniformName = QString("spheres[%1].material.type").arg(i);
            glUniform1i(rayTracingShaderProgram.uniformLocation(uniformName.toStdString().c_str()), modelsType[i]);
        }

        glUniform1i(rayTracingShaderProgram.uniformLocation("numSpheres"), 3);
    ...
    rayTracingShaderProgram.release();
